...
##        ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝        ##
##                                                                ##
##        Coded by Brycen Addison, 2018                           ##
##        Version 0.4                                             ##
##                                                                ##
##        Changelog:                                              ##
##           Added binary                                         ##
##           Added icon                                           ##
##           Made highscore.archer to work with exe               ##
##           Changed main.py to archer.py                         ##
##           Formatted document                                   ##
##           Added anticheat                                      ##
##                                                                ##
####################################################################
...
class game:  # Game functions
    ...
    def checkpause():  # Checks if the game should be paused and then pauses the game
        ...
        if paused:
            ...
            game.textdisplay(
                int(game.width / 20),
                "press tab to unpause", (game.width / 2),
                (game.height / 1.7))
            pygame.display.flip()
    ...
    def saveScore(n: int):  # Updates high score file
        with open("highscore.archer", "w") as savefile:
            savefile.write(f"{n}\n{round(time.time())}")

    def loadScore():  # Checks if the save file can be imported
        global highScore
        try:
            file = open("highscore.archer", "r")
            score = int(file.readline())
            savetime = round(float(file.readline()))
            file.close()
            if round(os.path.getmtime("highscore.archer")) == savetime:
                return score
            else:
                game.saveScore(0)
                return 0
    ...
    def checkBest():  # Replaces high score if beaten
        global highScore
...
class player(pygame.sprite.Sprite):  # Player class
    def __init__(self):
        ...
        # Your base speed after changing directions
        self.regularVelocity = 0 * game.width / 360
        self.velocityCap = 3 * game.width / 360  # Maximum regular player velocity
        # How fast your velocity rises after moving in one continual direction
        self.velocityGain = 0.2 * game.width / 360
        # How fast the player loses speed and changes directions
        self.velocityLoss = 0.25 * game.width / 360
        # What your speed changes to when you dash
        self.dashVelocity = 10 * game.width / 360
        ...
        # Sets coordinate x for circle draw
        self.circlex = int(0.5 * self.width)
        # Sets coordinate y for circle draw
        self.circley = int(0.5 * self.height)
        # Sets radius for circle draw
        self.radius = int((self.width+self.height) / 4 - 1)
        # Creates surface to draw circle onto
        self.art = pygame.Surface((self.width, self.height))
        self.art.fill((255, 255, 255, 255))  # Makes surface white
        pygame.gfxdraw.aacircle(self.art, self.circlex, self.circley,
                                self.radius, (50, 100, 255))  # Draws outer circle AA
        pygame.gfxdraw.filled_circle(
            self.art, self.circlex, self.circley, self.radius, (50, 100, 255))  # Draws outer circle
        if self.drawInnerCircle == True:
            pygame.gfxdraw.aacircle(
                self.art, self.circlex, self.circley,
                int(self.radius * (3 / 4)),
                (0, 0, 0))  # Draws inner circle AA
            pygame.gfxdraw.filled_circle(
                self.art, self.circlex, self.circley,
                int(self.radius * (3 / 4)),
                (0, 0, 0))  # Draws inner circle
    ...
    def move(self):
        ...
        if self.isDash:
            ...
            if self.shiftValue == 1 and (
                    move[pygame.K_a]
                    or move[pygame.K_LEFT] or
                    move[pygame.K_d] or move[pygame.K_RIGHT]) and (
                    self.dashUntilReset == 0):
                self.isDash = True
        ...
        if not(
                move[pygame.K_d]
                or move[pygame.K_RIGHT] or
                move[pygame.K_a] or move[pygame.K_LEFT]):
            if self.movingLeft > 0:
                self.movingLeft -= self.velocityLoss
                if self.x > game.boundary:
                    self.x -= self.velocity + self.movingLeft
    ...
    def shoot(self):  # Checks for keyboard inputs to make the player shoot
        ...
        if self.spaceValue == 1 and self.shotCount == 0:
            arrow = Arrow(
                self.x + self.width / 2, self.y, pygame.mouse.get_pos()[0],
    ...
    def render(self):  # Displays player object on starting screend
        pygame.gfxdraw.aacircle(
            window, int(self.x + self.circlex),
            int(self.y + self.circley),
            self.radius, (50, 100, 255))  # Draws outer circle AA
        pygame.gfxdraw.filled_circle(
            window, int(self.x + self.circlex),
            int(self.y + self.circley),
            self.radius, (50, 100, 255))  # Draws outer circle
        pygame.gfxdraw.aacircle(
            window, int(self.x + self.circlex),
            int(self.y + self.circley),
            int(self.radius * (3 / 4)),
...
class Arrow(pygame.sprite.Sprite):  # Player Projectile
    ...
    def update(self):
        pygame.gfxdraw.aacircle(self.image, self.circlex,
                                self.circley, self.radius, (0, 0, 0))
        pygame.gfxdraw.filled_circle(
            self.image, self.circlex, self.circley, self.radius, (0, 0, 0))
...
class Target(pygame.sprite.Sprite):  # Basic enemy class
    ...
    def update(self):  # Object commands to be parsed every frame
        ...
        # The enemy stops at a random point 10 and 40 percent down the screen
        ...
        if self.speed == 0:  # Starts shooting when enemy is stopped
            if self.nextShotTimer == self.fireRate:  # Shoots when shot cooldown is up
                avoid = Avoid(
                    ...
                    game.height / 64)  # Creates projectile
                # Adds projectile to group of sprites to be refreshed every frame
                all_sprites.add(avoid)
                # Adds projectile to group of enemy projectiles
                avoids.add(avoid)
...
class BuffTarget(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # The speed that the enemy comes from the top of the screen
        self.speed = game.width / 64
        # Holds the number of frames since last shot # Initial values of the object
    ...
    def update(self):  # Object commands to be parsed every frame
        ...
        if self.speed == 0:  # Starts shooting when enemy is stopped
            if self.nextShotTimer == self.fireRate:  # Shoots when shot cooldown is up
                ...
                self.nextShotTimer = 0  # Resets shot cooldown after shot
            self.nextShotTimer += 1  # Adds to shot cooldown
        # Detects hits between player shots and enemy
        hits = pygame.sprite.groupcollide(bufftargets, arrows, False, True)
        for hit in hits:
        ...
        self.image.blit(
            self.outlinebox,
            (int(self.width / 16 + 0.5),
             int(self.height / 16 + 0.5)))  # Refreshes color of cube
...
class MovingTarget(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # Initial boundary that a moving target can move to the left
        self.movingMaxLeft = random.randint(game.boundary, game.width/4)
        # Initial boundary that a moving target can move t the right
        self.movingMaxRight = random.randint(
            game.width*0.75, (game.width-game.boundary))
        self.nextShotTimer = 0
        if random.randint(
                0, 1) == 1:  # Randomly determines whether the cube should start moving towards the left or the right
...
class Avoid(pygame.sprite.Sprite):
    ...
    def update(self):
        pygame.gfxdraw.aacircle(self.image, self.circlex,
                                self.circley, self.radius, (255, 0, 0))
        pygame.gfxdraw.filled_circle(
            self.image, self.circlex, self.circley, self.radius, (255, 0, 0))
        dist = max(1, math.hypot(self.dx, self.dy))
        self.vx = self.speed * (self.dx / self.dist)
        self.vy = self.speed * (self.dy / self.dist)
        self.x += self.vx
...
class AirstrikeTarget(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.minigunCounter == self.fireRate:
            ...
            avoid = MinigunShot(self.rect.x + self.width / 2,
                                self.rect.y + self.height + game.height / 64)
            all_sprites.add(avoid)
            avoids.add(avoid)
...
class MinigunShot(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        pygame.gfxdraw.filled_circle(
            self.image, self.circlex, self.circley, self.radius, (0, 0, 0))
        pygame.gfxdraw.aacircle(
            self.image, self.circlex, self.circley, self.radius,
            (255, 255, 0))
        pygame.gfxdraw.filled_circle(
            self.image, int(1 / 8 + self.circlex),
            int(1 / 8 + self.circley),
            int(7 / 8 * self.radius),
            (255, 255, 0))
...
class BombTarget(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image.blit(
            self.tickbox,
            (int(self.width / 4 + 0.5),
             int(self.height / 4 + 0.5)))
        self.rect = self.image.get_rect()
        self.rect.x = random.randint(
            int(game.boundary),
            int(game.width - self.width - game.boundary))
        self.rect.y = random.randint(int(-game.height), int(-self.height))
        self.speed = game.width / 64
        self.tick = False
        self.exploded = False
        ...
        if self.speed == 0:
            ...
            if self.exploded == True:
                if (self.shotTimer % 5) == 1:
                    ...
                    avoids.add(avoid1)
                    avoid2 = SeekingAvoid(
                        self.rect.x - game.height / 64, self.rect.y + self.height / 2, 'left')
                    all_sprites.add(avoid2)
                    avoids.add(avoid2)
                    avoid3 = SeekingAvoid(
                        self.rect.x + self.width / 2, self.rect.y - game.height /
                        64, 'up')
                    all_sprites.add(avoid3)
                    avoids.add(avoid3)
                    avoid4 = SeekingAvoid(
                        self.rect.x + self.width + game.height / 64, self.rect.y + self.height / 2, 'right')
...
def display():
    ...
    counter = font.render(
        f"Health: {archer.health} Hits: {archer.hits} Best: {highScore} Cubes: {cubes} FPS: {int(100*clock.get_fps())/100}",
        True, (0, 0, 0))
...
def intro():
    ...
    while intro:
        ...
        game.text.still(
            int(game.width / 36),
            "brycen addison, 2018", (game.width / 6),
            (game.height / 1.01))
        game.text.still(
            int(game.width / 16),
            "press enter to start", (game.width / 2),
            (game.height / 1.5))
        archer.render()
        all_sprites.update()
...
def loop():
    global alive, spawnRate, all_sprites
    # Main game loop
    running = True
    alive = True
    while running:
        clock.tick(game.fps)
        # Checks if game is being quit, will close game if true.
        game.checkquit()
        game.checkpause()
        game.update()
        hitDetect()
        ...
        if alive == False:
            game.text.still(
                int(game.width / 4),
                "you lost", (game.width / 2),
                (game.height / 3))
            game.text.still(
                int(game.width / 16),
                f"your score: {archer.hits}", (game.width / 2),
                (game.height / 2.2))
            game.text.still(
                int(game.width / 16),
                f"best score: {highScore}", (game.width / 2),
                (game.height / 2))
            game.text.still(
                int(game.width / 20),
                "press enter to play again", (game.width / 2),
                (game.height / 1.7))
            game.text.still(
                int(game.width / 20),
                "or escape to quit", (game.width / 2),
                (game.height / 1.6))
            pygame.display.flip()
            while alive == False:
                clock.tick(60)
...
# Set game caption/resolution/icon
